...
from parser.AstTree import Node, ASTree, ConnectSame, ConnectWithOps
...
from parser.Scanner import Position
...
class Parser( object ):
   ...
   def Defenition( self ):
      self.Expect(1)
      name = self.token.val; name_pos = self.token.pos 
      params = self.Params()
      self.Expect(4)
      type = self.Type()
   ...
   def Type( self ):
      ...
      if self.la.kind == 8:
         self.Get( )
         val = self.token.val; end_pos = self.token.pos 
         if (self.la.kind == 9):
            self.Get( )
            if (self.StartOf(2)):
   ...
   def CompoundStatement( self ):
      tree = Node(t='GRAMMAR_CONSTRUCTION') 
      self.Expect(14)
   ...
   def IfStatement( self ):
      self.Expect(20)
      tree = Node(value='KOLI', t='KEYWORD', start_pos=self.token.pos - 1, end_pos= self.token.pos + len("KOLI") - 1) 
      self.Expect(5)
   ...
   def ReturnStatement( self ):
      self.Expect(18)
      tree = Node(value="VOZDAT", t='KEYWORD', start_pos=self.token.pos - 1, end_pos=self.token.pos + len("VOZDAT") - 1) 
      if (self.StartOf(2)):
         expr_tree = self.Expression()
   ...
   def WhileStatement( self ):
      self.Expect(19)
      tree = Node(value='DOKOLE', t='KEYWORD', start_pos=self.token.pos - 1, end_pos=self.token.pos + len("DOKOLE") - 1) 
      self.Expect(5)
      expr_tree = self.Expression()
      self.Expect(6)
      state_tree = self.Statement()
   ...
   def Primary( self ):
      if self.la.kind == 1:
         self.Get( )
         prim = Node(value=self.token.val, t='KEYARG', start_pos=self.token.pos - 1, end_pos= self.token.pos + len(self.token.val) - 1) 
      elif self.la.kind == 3:
         self.Get( )
         prim = Node(value=self.token.val, t='STRING', start_pos=self.token.pos - 1, end_pos= self.token.pos + len(self.token.val) - 1) 
      elif self.la.kind == 2:
         self.Get( )
         prim = Node(value=self.token.val, t='CONSTANTS', start_pos=self.token.pos - 1, end_pos= self.token.pos + len(self.token.val) - 1)  
      elif self.la.kind == 5:
         self.Get( )
         start_pos = self.token.pos 
         tree = self.Expression()
   ...
   def FunctionCall( self ):
      res = Node(value=self.token.val, t='FUNCTIONS', start_pos=self.token.pos - 1, end_pos=self.token.pos + len(self.token.val) - 1) 
      self.Expect(5)
      if (self.StartOf(2)):
   ...
   set = [
      ...
      [x,T,T,T, x,T,x,x, T,x,x,T, T,T,T,x, x,x,T,T, T,x,x,x, x,x,x,x, x,T,T,T, x,x,x,x, x,x,T,x, x],
